...
import { Save, Loader2, Phone, Mail, MapPin, Globe, MessageCircle, Send, AtSign, Eye, EyeOff, ExternalLink } from 'lucide-react';
...
export default function AdminFooterPage() {
  ...
  const fetchContent = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const existingLinks = data.sections?.social_links || [];
        const platforms = [
          'facebook', 'instagram', 'twitter', 'linkedin', 'github', 
          'whatsapp', 'telegram', 'youtube', 'tiktok', 'messenger', 
          'snapchat', 'line', 'skype', 'globe'
        ];
        
        const social_links = platforms.map(platform => {
          const existing = existingLinks.find((l: any) => l.platform === platform);
          return {
            platform,
            url: existing?.url || '',
            visible: existing ? (existing.visible !== undefined ? existing.visible : true) : false
          };
        });

        setContent(data.sections ? { ...data.sections, social_links } : {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/content', {
        ...
        body: JSON.stringify({ page: 'footer', sections: content }),
      });
      if (res.ok) {
  ...
  const updateSocialLink = (index: number, field: string, value: any) => {
    const newSocialLinks = [...content.social_links];
    newSocialLinks[index][field] = value;
    setContent((prev: any) => ({ ...prev, social_links: newSocialLinks }));
  };
  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-slate-900 dark:text-white">Footer Content</h1>
          <p className="text-slate-500 mt-2 font-medium">Manage the contact information and social media links in your footer.</p>
        </div>
        <Button
          onClick={handleSave}
          disabled={saving}
          ...
          <section className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 shadow-xl shadow-indigo-500/5 border dark:border-slate-800 space-y-8">
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <Label className="text-sm font-black uppercase tracking-widest text-slate-400 ml-1">Footer Description</Label>
                <Textarea
                  value={content.footer_description}
                  onChange={(e) => updateField('footer_description', e.target.value)}
                  className="rounded-2xl border-2 focus:ring-indigo-600 min-h-[100px] font-bold"
                  placeholder="Enter footer description..."
                />
              </div>

              <div className="space-y-3">
                <Label className="text-sm font-black uppercase tracking-widest text-slate-400 ml-1">Office Address</Label>
                <div className="relative group">
                  ...
                  <Input
                    ...
                    className="pl-12 py-7 rounded-2xl border-2 focus:ring-indigo-600 font-bold"
        ...
        <div className="space-y-10">
          <section className="bg-white dark:bg-slate-900 rounded-[3rem] p-10 shadow-xl shadow-indigo-500/5 border dark:border-slate-800 space-y-8">
            ...
            <div className="grid grid-cols-1 gap-6">
              {content?.social_links && Array.isArray(content.social_links) ? content.social_links.map((link: any, index: number) => {
                const Icon = SOCIAL_ICONS[link.platform] || Globe;
                return (
                  <div key={link.platform} className="bg-slate-50 dark:bg-slate-950/50 p-6 rounded-[2rem] border-2 border-transparent hover:border-indigo-500/20 transition-all group">
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center space-x-3">
                        <div className="w-12 h-12 bg-white dark:bg-slate-900 rounded-xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform duration-300">
                          <Icon className="h-6 w-6 text-indigo-600" />
                        </div>
                        <div>
                          <p className="font-black text-slate-900 dark:text-white capitalize">{link.platform}</p>
                      ...
                      <div className="flex items-center space-x-2">
                        {link.url && (
                          <Button
                            ...
                            size="icon"
                            asChild
                            className="rounded-xl hover:bg-indigo-50 dark:hover:bg-indigo-950/30 text-indigo-600"
                          >
                        ...
                        <div className="flex items-center space-x-2 bg-white dark:bg-slate-900 px-3 py-1.5 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800">
                          {link.visible ? <Eye className="h-3.5 w-3.5 text-indigo-600" /> : <EyeOff className="h-3.5 w-3.5 text-slate-400" />}
                          <Switch
                            checked={link.visible}
                            onCheckedChange={(checked) => updateSocialLink(index, 'visible', checked)}
                    ...
                    <div className="relative group/input">
                      <Globe className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 group-focus-within/input:text-indigo-600 transition-colors" />
                      <Input
                        value={link.url}
                        onChange={(e) => updateSocialLink(index, 'url', e.target.value)}
                        className="pl-10 py-6 rounded-xl border-2 focus:ring-indigo-600 font-bold text-sm bg-white/50 dark:bg-slate-900/50"
                ...
                <p className="text-slate-500 font-bold italic">No social links configured.</p>
              )}
            </div>
          </section>
        </div>
